# -*- coding: utf-8 -*-
"""GMM_experiment_mu.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/13IeoBqZuT6vTeA6mn6qV95fK-WWf82TM
"""
...
if generate:
    ...
    for d in tqdm(d_mu_list):
        ...
        for _ in range(n_mc):
            ...
            gmm_means = np.array([np.linspace(d,d*n_models,n_models) for i in range(n_models)])
            n_gmm_components=len(gmm_means[0])
            gmm_sigmas = np.ones((n_models,n_gmm_components))
        ...
        parameter_dict[d] = parameter_list
        results_dict[d] = Parallel(n_jobs=-1)(delayed(f)(x) for x in parameter_dict[d])
...
df = pd.read_pickle('data_mu.p')

def threshold(Stability):
    Stability.reverse()
    for index, num in enumerate(Stability):
        if num != 1.0:
            if index+1>20:
                return T-index
            else:
...
for d in np.linspace(1,6,11)[1:]:
    ...
    for v in range(50):
        ...
        for t in range(1,IDs.shape[0]):
            Stability.append(np.sum(IDs.iloc[t,:]==IDs.iloc[t-1,:])/IDs.shape[1])
        t_threshold.append(threshold(Stability))

    T_threshold.append(t_threshold)

result = pd.DataFrame(T_threshold)
Mean = result.mean(axis=1)
SE = result.std(axis=1)/np.sqrt(50)
Top = Mean + 5*SE
Bottom = Mean - 5*SE
