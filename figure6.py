...
if __name__ == '__main__':
    ...
    one_silo_prop = stat['#Silos_T'].astype(bool).mean()
    
    print(f'Proportion of systems with 1 silo: {one_silo_prop}')

    # Example system
    gmm_weights_history = df[30][p][k][0]
    IDs = []
    for t in range(T+1):
        ids = []
        for i in range(n_agents):
    ...
    ax = IDs.plot(y=IDs.columns, 
                          title=f'GMM Interaction, p={p}, k={k}, RAG size={rag_size}', 
